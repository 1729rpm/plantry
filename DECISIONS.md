...
## 2026-08-11  Engine v4 spec authored from the eaten-record reconstruction; three parameter calls fixed inside it  (features/engine-v4.md)
**Context:** Rajat approved the debate-derived reconstruction (frequency-first chooser, simplified templates, pairsWith, timesPerWeek dial) and its three recommendations (one exploration slot; hand-placed 2x weighting as phase 2; rolling 10-week window), and asked for a pre-implementation spec. The converged debate left three implementation points unfixed.
**Decision (EM, recorded in the spec):** (a) standalone-plate days are fixed Mon+Tue rather than recency-selected; the eaten record puts standalone lunches overwhelmingly early-week, fixed days are predictable, and a third is one swap away. (b) The weekly exploration slot is Friday's Indian-plate companion position: novelty enters at the lowest-stakes position, one tap to remove. (c) weekArchive rows gain the optional per-dish `source` field in v4 itself (finalize passthrough) even though the 2x weighting is phase 2, so the chosen-vs-tolerated data accrues from the first v4 week.
**Reversibility:** high (a and b are one-line template constants; c is an additive optional field). **Right-size:** fixes only what implementation cannot proceed without; everything speculative (cadence, satiety budgets, season-scoped windows) is in the spec's rejected list with re-entry conditions.

## 2026-08-17  Engine v4 falsified by simulation, amended to v4.1; three design calls taken by the EM  (features/engine-v4.md §10)
**Context:** the approved v4 spec was prototyped in an isolated worktree and simulated for 25 self-feeding weeks, then audited by three independent evaluators. Eleven defects surfaced, two critical. The implementation was faithful (templates 100%, 7 of 8 plate rules 100%, ranker reproduced 250/250 by independent re-derivation), so every defect was in the specification. Rajat then reviewed the simulated menus and added two more findings.
**Decisions (Rajat):** whole-day prep budget of 120 minutes plus a 6-item backstop, replacing the 5-weekday / 3-Saturday item cap. Both sit at the envelope of the eaten record (busiest observed day is exactly 120 minutes; largest is 5 items), so the time budget binds and the item rule is a backstop.
**Decisions (EM, recommendation was on the record first, open to veto at PR review):**
(a) **Saturating frequency count** (`min(eatenCount, 3)`) as the fix for defect D1. Nothing in v4 could dislodge an incumbent, because the 7-day guard delays a leader by exactly 8 days rather than suppressing it, and with deterministic tiebreaks every position converged to a fixed cycle (whole menu period 20; week 21 was 85 to 94 percent identical to week 1). Saturation lets proven dishes tie at the cap so the longest-unused tiebreak restores rotation, which is what the household actually does.
(b) **Withdraw the `timesPerWeek` favorites dial** (D8). It placed twice on 0 of 25 weeks, its supporting evidence was always one week in six, and it was the direct cause of twelve consecutive-day kadhi repeats.
(c) **Restore within-week protein-family diversity** (D9). Both debate agents agreed to retire it and the EM endorsed that; the simulation falsified it, producing chicken on 72 percent of days and zero prawn or mutton across six months.
**Also corrected, EM errors in the original spec:** the claim that the Option C breakfast form "occurred zero times" was counted over the six app weeks only and is false over the eleven-week record (paneer bhurji with plain paratha 2026-06-08; anda bhurji with toast 2026-05-04), which had stranded eleven breakfast dishes; the claim that international lunches "usually" carry a protein side (6 of 13 were eaten alone); and five of six `pairsWith` seed pairs were written from the eaten record without validating them against the composition rules.
**Reversibility:** high (all are rule constants or single clauses; the spec retains §3 as the record of the superseded design). **Right-size:** the two critical defects are rule interactions that no amount of reasoning about rules in isolation would have caught, which is why the 25-week self-feeding simulation is now a mandatory gate (§10.8) rather than a one-off exercise.

## 2026-08-17  Engine v4.1 stream shape: Activation first, A before B, C in parallel  (coordination/active-streams.md H8, H9)
**Context:** the phase touches four EM-owned hotspot files plus the whole selection and composition core.
**Decision (EM):** a small Activation stream lands the spec, the PLAN row, the CLAUDE.md line and these decision entries first (hotspot H8), because the main dir cannot commit and all four files are EM-owned. Streams A (selection) and B (composition) are sequenced rather than parallel (hotspot H9): both touch `generateWeek.ts` and `docs/engine.md`, and A's ranking API is B's input. Stream C (data and backend) runs in parallel with A on disjoint lanes. Stream D is EM-run verification against §10.8 and merges nothing.
**Why not parallelize A and B:** the two share the engine's hottest file and the coupling is semantic, not incidental. Parallel authoring would produce a rebase fight on every commit for no wall-clock gain.
**Reversibility:** high (coordination only). **Right-size:** two ledger rows instead of serializing the whole phase.

## 2026-08-17  Decision #8 amended: the reason is optional on every menu change  (docs/product.md §6)
**Stream:** feat/optional-change-reasons (standalone)
**Context:** Rajat asked that no change to the week require a comment, and that every place demanding one be found and relaxed. Decision #8 had made a written reason mandatory on save-for-next-week and, by extension, on the other fast-loop writes; only `swapDish` (relaxed 2026-06-15) and the Explore dislike (decision on 2026-06-24) were already optional.
**Options considered:** (a) relax the six remaining mutations plus the shared dialog, so a reason is uniformly optional; (b) relax the dialog only and leave the backend guards, so a direct or future caller still throws; (c) relax skip and restore only, keeping dish-level edits gated.
**Chosen:** (a). A guard the UI can no longer trigger is a trap for the next caller, not a safety net, so the validator and the affordance move together. Uniformity also removes the thing that made the old behaviour confusing: swap was optional while add, delete, skip, restore, and custom were not, with no principle a user could infer. The reason still reaches `manualChanges` whenever it is offered, which is all the slow loop ever needed.
**Two EM calls inside that frame:** (i) the day-level "Note for the weekly review" composer keeps requiring text. It is not attached to a menu change and its text is the entire payload, so an empty post would queue a blank row for the slow loop; product.md already described it as carrying no required reason of its own. (ii) The shared `ReasonDialog` drops `autoFocus` on its textarea. With the field optional, autofocusing it raised the keyboard over the confirm button on a field most taps now skip; the Sheet primitive still moves focus into the panel, so the §16 focus invariant is verified intact (`sheet__panel` holds focus on open).
**Reversibility:** high. The mutations are function-body edits with no schema change (`reason` stays `v.string()`, empty means none given), and the dialog is one boolean. Re-tightening is a small edit in both places, though any rows written with `""` would then predate the rule.
**Right-size check (per `docs/product.md` §4):** problem size is a friction tax on the most-used flow in the app; fix level is the shared component plus six validators, no new abstraction; generality: none added, the existing `swapDish` behaviour is simply extended to its five siblings.

## 2026-08-17  Day-comment removal scoped as a full-channel removal, sequenced ahead of engine-v4.1 C  (coordination/active-streams.md H10)
**Context:** Rajat flagged the "NOTE FOR THE WEEKLY REVIEW" card in the day editor as unused and asked for the experience and the code both removed. He asked to confirm first that no general comments UI would go with it. Verification: the app has four free-text inputs, and only this one writes the `comments` table; the swap reason, `ReasonDialog`, and dislike reason all write `manualChanges` or `dishDislikes` and are untouched. Two references suggested a second comment surface (`index.css:1256` naming a "comment" sheet, `docs/engineering.md:410` listing one in the crawl flows); both are stale naming for `ReasonDialog`, and no `CommentSheet` component exists.
**Decision (Rajat, after the confirmation):** full removal, including dropping the Convex `comments` table.
**Decisions (EM):**
(a) **Remove the whole channel, not just the write path.** `DayCommentCard` is the only writer anywhere in the app, so the mutation, the two queries, the table, the slow-loop wiring, and the fixture all become unreachable the moment the card goes. Leaving them would be exactly the dead-code residue the request was aimed at. The slow loop keeps three signal channels (`manualChanges`, `dishDislikes`, `incidents`).
(b) **Relocate `markIncidentsResolved` before deleting `app/convex/comments.ts`.** That file holds three internal mutations and only two of them are comment-related; the third serves the incidents channel and is called by `scripts/slow-loop-mark-applied.mjs`. It moves to `app/convex/incidentsMutations.ts`. Deleting the file wholesale would have silently broken the slow-loop mark-applied Action, which is the highest-risk step in the stream and is called out as such in the brief.
(c) **Rewrite the comment-derived right-size guidance rather than deleting it.** `.claude/commands/slow-loop.md` lines 62-63 and 71 ("a single comment is almost never a tag", "comments arrive sycophantically by nature") are anti-sycophancy discipline written against comments but true of every signal channel. They get re-pointed at `manualChanges` and `dishDislikes`, not dropped.
(d) **Branch prefix `feat/` over `chore/`.** §2 reserves `chore/` for tooling, deps, and CI; this changes product behavior and canonical specs, so it is not a chore even though it only subtracts.
(e) **This stream merges before engine-v4.1 C** (hotspot H10, `app/convex/schema.ts`). It is not an engine-v4.1 stream, so the §10.8 verification gate does not hold it, while C is gated and therefore merges later and owns the rebase. The two edits are disjoint (this drops the `comments` table; C touches the favorites and week tables), so the brief forbids any incidental reformatting of `schema.ts` that would turn C's mechanical rebase into a painful one.
**Open:** whether the table drop is one PR or two depends on the prod `comments` row count, which needs Rajat's per-action approval to read. Empty means drop outright; non-empty means ship a `wipeComments` migration first, run it, then drop in a follow-up PR (the pattern PR #224 used for `nextWeekQueue`).
**Reversibility:** low for the prod rows once wiped (Rajat accepted this), high for the code. **Right-size:** one stream, one channel, no adjacent cleanup of the Changes feed or the slow-loop docs beyond what removing the channel forces.

## 2026-08-17  Amendment: day-comment removal resequenced behind optional-change-reasons; hotspot ids renumbered  (coordination/active-streams.md H11, H12)
**Context:** after the entry above was written, a `git worktree list` surfaced a fifth worktree, `feat/optional-change-reasons`, making change reasons optional across 9 files with no commits, no PR, no brief, and no registry row. It was not visible to the pre-spawn registry check. Rajat confirmed it is a legitimate separate change. A concurrent session registered it while this session was working, which briefly produced a duplicate stream row and two conflicting `H10` ids; both were reconciled here.
**Decision (EM):** optional-change-reasons merges first and the day-comment removal owns the whole rebase (H12), reversing the usual instinct to let the bigger change go first. The removal has not started; the reasons stream is materially written. Rebasing started work around unstarted work is backwards, and §11.3's later-merging-session rule then puts the rebase on the removal by construction.
**Decision (EM):** the removal engineer does `docs/product.md` and `docs/engineering.md` LAST, after rebasing onto merged optional-change-reasons. The `DayScreen.tsx` overlap is mechanically disjoint (skipped-day reason block at ~181 versus `DayCommentCard` at 395-475), but the two canonical docs are not: both rewrite the same §2/§3 and §3/§5 prose about reasons and comments. Sequencing the doc edits last turns a paragraph-level conflict resolution into writing the surviving prose once.
**Renumbering:** the schema.ts hotspot moves from H10 to H11 (a concurrent session had already claimed H10 for `weekMutations.ts`); the new removal-versus-reasons hotspot is H12.
**Flagged to Rajat, not decided:** removing the comments channel and making reasons optional in the same week costs the slow loop one of its four signal inputs and thins a second. Each change is defensible alone; the combined reduction in review signal is larger than either PR shows on its own and is his call, not the EM's.
**Reversibility:** high (coordination only). **Right-size:** two ledger rows and a reordering of one brief's working order, rather than blocking either stream.

## 2026-08-18  Comment channel fully retired: prod wiped, table dropped  (#231, #234)
**Context:** Rajat authorised the destructive prod wipe and the follow-up drop in one instruction, closing the two-step sequence #231 opened.
**Actions (EM, on Rajat's explicit go-ahead):** ran `npx convex run --prod migrations:wipeComments` on `disciplined-chameleon-263` (returned `{ commentsDeleted: 2 }`), verified the table reported no documents, then opened and merged **#234** dropping the `comments` table from `schema.ts` and deleting the throwaway `app/convex/migrations.ts`. `Deploy Convex` SUCCESS on `774c2bc`. Post-deploy prod read confirms the table is gone and `manualChanges` still reads normally.
**Decision (EM):** kept the drop as its own PR rather than folding it into #231, per the PR #224 `nextWeekQueue` precedent. Convex validates existing rows against the new schema on deploy, so a single-PR drop would have depended on the table being empty at deploy time, a property no branch controls. The cost was one extra small PR; the benefit was that the drop could not fail the deploy.
**Verification standard applied:** the removal was checked at four independent layers rather than by grep alone: 620 engine + 116 web tests, an EM-scripted crawl (57/57, Chromium and WebKit, all four tabs, all six days opened individually since the card rendered on each), a built-bundle string assertion, and a live interactive check that the three surviving free-text inputs still render (`ReasonDialog` placeholder confirmed as "Why this change? (optional)", which also re-verified #227). The crawl found two things grep could not: the day-open affordance is `aria-label="View <Day>"`, not an Edit button, and Monday is currently a skipped day in the live week.
**Residual, documentation only:** `CLAUDE.md` line 7, `docs/development.md` §5 / §6 steps 3-4-6 / §9, and `docs/PLAN.md` line 20 still reference queued comments. All are EM-owned or reconciliation-owned files that a shipping stream should not edit; they are recorded in the #231 CHANGELOG `Updated:` line as the queue for `/reconcile-docs` and `/reconcile-ops`.
**Reversibility:** none for the 2 prod rows (both test garbage, accepted); the schema and code are recoverable from git history. **Right-size:** two PRs and one prod mutation, no adjacent cleanup.

## 2026-08-18  Maintenance sitting scoped to the operational layer; canonical reconcile deferred behind two open PRs  (PR #233)
**Stream:** docs/maintenance-2026-08-18 (EM-run)
**Context:** the maintenance markers had all read 2026-07-14 for five weeks. A full sitting would run `/reconcile-docs`, `/reconcile-ops`, and the retro intake. But `docs/product.md` and `docs/engineering.md` are owned by open PR #231 (day-comment removal) and `docs/engine.md` by open PR #229 (engine v4.1 Stream A), and both carry exactly the drift the 2026-07-15 `Updated:` lines name.
**Decisions (EM):**
(a) **Run `/reconcile-ops` and the retro intake; do not run `/reconcile-docs`.** Rewriting a canonical doc that an open PR is mid-rewrite of would hand that PR a paragraph-level conflict on files it already owns, for drift that is five weeks old and can wait two days. `last_reconcile_ops` and `last_retro` move to 2026-08-18; `last_reconcile` stays at 2026-07-14 so the next pass still sees the full queue.
(b) **Do not build the spec-parity CI check, even though three docs claim it exists.** `docs/engine.md` §13 describes two checks that were never written, and the v4.1 verification found the consequence (a dead rule 7 that drifted unnoticed). The right-sized move in a maintenance pass with three PRs open and a phase mid-flight is to stop the docs lying, not to add a merge-blocking gate that could red them. Building it is already required work in `features/engine-v4.md` §15.5.
(c) **Fix the CI branch filter now rather than queueing it.** Unlike (b) this only adds coverage: `pull_request` loses its `branches: [main]` filter, so a stacked stream is gated. It cannot red an existing main-targeting PR, and Stream B (#230) already merged with no CI run at all under the old filter.
(d) **Treat the main directory's `features/engine-v4.md` as canonical over the three worktree copies.** Four copies existed, all uncommitted and divergent. The main-dir copy was verified line-for-line as a strict superset of the Stream B and Stream E copies before commit, so no stream-authored amendment was discarded.
(e) **Restore the dropped `DECISIONS.md` entry rather than committing the working copy.** The working copy had lost the merged "Decision #8 amended" entry while gaining two new ones. The ledger is append-only, so the commit is HEAD plus the two new entries, a pure addition.
(f) **Leave `docs/CHANGELOG.md`'s 2026-08-17 optional-reason entry without its PR number.** The entry format asks for a PR reference and it has none (#227 did not exist when the entry was written). `MAINTENANCE.md` §7.5 puts the append-only ledgers out of scope for reconciliation passes, and that rule is worth more than the missing token. Flagged, not edited.
**Flagged to Rajat, not decided:** removing the stray root `design_handoff_wishlist_favorites/` (a verified byte-identical duplicate of the archived copy), pruning the merged worktrees and the stale local and remote branches, and the four un-promoted custom dishes from the week of 2026-08-10, which cannot be authored without entering Stream C's `data/dishes/` lane.
**Reversibility:** high throughout (docs, config, and one CI trigger; every deletion proposed rather than taken). **Right-size:** one PR, no new abstraction, three brief lines standing in for three process-doc edits that would otherwise need the deferred canonical pass.

## 2026-09-04  Engine v6 round 3 scope settled after the D1 to D12 debate  (features/engine-v6-decision-brief.md, features/review-7-v6-decision-debate.md)
**Stream:** cross-stream (no stream open; pre-planning)
**Context:** the v6 record-matching engine has two read-only dry runs and six reviews behind it and nothing implemented. The EM wrote a decision brief (D1 to D12) and Rajat asked for a three-round debate between a simplicity advocate and a coverage advocate before planning. The two sides converged on every item except four, which Rajat settled directly. Every number below traces to the review files; nothing here is new measurement.
**Decisions (Rajat, on the four irreducible items):**
(a) **Novelty is lunch-only.** One exploration pick a week, at weekday lunch; the spanning breakfast-plus-lunch slot is not shipped. Breakfast novelty comes from Rajat's own swaps.
(b) **No Saturday exploration door.** Saturdays rotate through the treats the household has eaten (11 or 12 after promotion and refresh); the pool grows only by swaps and custom dishes. The thin-pool door text stays in the debate file as a contingency if the refreshed pool is under 8.
(c) **The engine admits Winter fruits itself when the eaten pool runs short.** The fruit slot's exhausted-pool fallback draws by least-recently-served over every Active in-season fruit, candidates included. No new state; self-extinguishing once a fruit is eaten.
(d) **Avocado toast every week stands.** The favorites-table guarantee is a product contract, not an engine matter, even though the household ate it 6 of 8 weeks.
**Decisions (EM, converged by both debaters and adopted):**
(e) **Amend v6 in place as round 3; no v7.** Every change extends the ledger; the chooser is untouched.
(f) **No slot-level ledgers (the brief's D2 is withdrawn).** Two deletions instead: the optional breakfast chutney slot is removed (chutney is dish-driven only; verified 15 of 15 household chutneys sit on paratha, chilla, or standalone boiled eggs, 0 of 21 elsewhere), and the cold-start seed applies only to dishes in structural pools. A reopen trigger is written into the spec: any optional slot more than 25 percent over its record presence on weeks 20 to 60 of the self-feeding run reopens a slot ledger for that slot only. The production dish-driven chutney rule names Chilla and Paratha; it is extended to standalone Boiled eggs (Category Keto) so the 4 standalone-egg chutney mornings stay servable.
(g) **Rates are per record occasion of the slot type, not per record week.** The household ate 44 of 48 planned days, so per-week rates overstate every family by about 9 percent; per occasion, chicken and mutton at 60 weeks fall inside the 25 percent bar. The gate compares per occasion served against per occasion eaten.
(h) **Exhausted structural pools fall back to the highest-rate dish not already placed this week**, ties by id, for every structural fill (star, carb, breakfast main, floor append, dry-protein partner). This is what returns specialty-roti overflow to plain roti. The harness reports negative-deficit fills per role.
(i) **Symmetric occasion-scoped lunch ledgers:** a dish has a Saturday ledger over its Saturday rows and a weekday ledger over its weekday rows, each charged by its own placements; Saturday-only and weekday-only dishes are absent from each other's pools, not present at rate zero. The dry-protein partner for a carb-forward international treat takes the Saturday accompaniment slot with precedence; the plate stays at three.
(j) **Dessert stays structural** with a recorded trigger (dessert-less Saturdays at 20 percent or more over 12 or more refreshed Saturdays) that flips one word in §5.4 to "optional" under the existing §3 rule.
(k) **Exploration affinity is scored within the candidate's own meal type**; the family governor is kept and a governor-off variant is measured for later deletion.
(l) **Exploration picks are assigned to days last** (D8 option b); the slot-level LRU tie-break is held in reserve until the round-3 anti-lock gate shows a weekday over half.
(m) **Three harness runs:** frozen rates (engine bias), self-feeding (drift), and a corrected replay of the record's swap-aways so §3 reconciliation executes at least once. A drift bound (weeks 40 to 60 within 10 percent of weeks 20 to 40, provisional) and a cold-start sweep (cap 1, 0.5, pool-level, structural-only; plus the §14.1 rate-formula variant, after which §14.1 closes). A slow-loop monthly table carries family rates per occasion, Saturday pool size, Saturday swap-outs, and the exploration swap-away rate.
(n) **Gate text:** Jaccard baseline re-measured by the harness method and the band set at baseline plus or minus 0.05; coverage gains "in which it is eligible"; international persistence counts weekday stars only; the thin-pool exemption covers the fruit twice-cap and the four-distinct floor; Saturday distinctness within min(8, pool size); seven added fidelity metrics (breakfast small-item presence, weekday companion presence, Saturday accompaniment presence, novelty per week, weekday lunches with no animal protein, Saturday plate shape, negative-deficit fills); the prep ceiling becomes "days over 120 reported, zero over 150" with no star-replacement repair.
(o) **Left alone, consciously:** metronomic cadence, no ordinary within-week repeats, the two-anchor rule, the Thursday anchor's strictness, the protein floor and the 74 versus 58 percent weekday non-veg rate (measured after the changes above, then revisited). The veg-led weekday lunch and the within-week favorite repeat are recorded as measured and unserved.
(p) **Process:** promote the six custom one-offs (dosa, atta halva, red sauce pasta, paneer manchurian, stuffed capsicum, cabbage matar aloo) and swap their slots to library ids before the read-only record refresh; apply the review-2 amendments to `household-menu-rules.md` first; content tasks run first and in parallel (Winter fruits added as Active dishes, since the library's Winter-eligible fruit set is exactly banana, papaya, pomegranate; soya and tofu deactivation; breakfast Dry retag; hummus category review); commit the thirteen untracked `features/` files; one clean-room review plus a resolution audit per round.
**Open, awaiting Rajat:** which Winter fruits the household eats (needed for the content task); whether hummus counts as a lunch main (it is Category Accompaniment and led 2 of 36 recorded weekday lunches).
**Verify in the harness before reading the round-3 gate:** whether a structural chutney fill charges the chutney ledger; why two round-2 aloo parathas carried no chutney under a dish-driven rule; how per-occasion accrual treats a skipped production day (accrue per planned occasion; a skip reduces only the record's occasion count); whether gate 4's category clause still fails if the refreshed Saturday pool is majority international.
**Reversibility:** high; nothing is implemented and the spec amendment is a text change. **Right-size:** the converged design is smaller than the EM's brief (one ledger deleted, one slot deleted, one door dropped); no per-family rule, no chicken ceiling, no row weighting.

## 2026-09-04  Phase 9 (engine v6) opened: spec amended in place, development plan written, v4.1 abandoned  (features/engine-v6-plan.md)
**Stream:** 0 (activation, EM)
**Context:** Rajat asked for the v6 spec to be updated to the debate outcome and for a comprehensive development plan for Opus-level engineer subagents, with no further prototype dry run before implementation. The plan is `features/engine-v6-plan.md`; the spec is `features/engine-v6.md` round 3; the rulebook carries the review-2 amendments.
**Decisions (Rajat):** (a) no prototype dry run; the §11 gate runs on the built engine on the integration branch and is the merge condition; (b) the four taste calls of the earlier entry stand (lunch-only novelty, no Saturday door, engine-admitted Winter fruits, weekly avocado toast).
**Decisions (EM):**
(a) **v6 amended in place as round 3, not v7.** Every change extends the ledger; the chooser is untouched.
(b) **An integration branch `feat/engine-v6`,** with every engine, backend, and docs stream PR targeting it and one squash merge to `main` after the gate. v4.1 showed stream-level green proves nothing for this engine. Content batches go to `main` directly and are merged into the integration branch (merge commits, never a rebase of the shared branch).
(c) **The deficit ledger is replayed from persisted data on every generation, not stored.** Replay from `currentWeek` rows plus a new additive `generatedPlan` field is linear in weeks, honors §10 determinism, needs no migration or repair path, and makes the gate's corrected run the production code path.
(d) **The cutover week is derived** (the earliest week carrying a `generatedPlan`), not an environment variable.
(e) **`currentWeek` rows are the record source, not `weekArchive`.** Finalize snapshots a week the household keeps editing; the as-eaten reconstruction already had to bypass it. `weekArchive` stays for provenance.
(f) **v6 lives in new modules under `engine/src/v6/`** so four engine streams have disjoint lanes and the v3 selection code is deleted in one cleanup stream instead of edited piecemeal.
(g) **Custom-pick re-pointing is an internal mutation that writes no `manualChanges` row.** It repairs the record; it is not a household edit.
(h) **The breakfast Dry retag is dropped as a content task.** The v6 breakfast-main pool admits Breakfast-time Dry dishes by definition; the bare carbs (Pav, Toast, Plain paratha) belong to no pool and carry no ledger, so they need no retag either.
(i) **Phase 8 is marked abandoned in `docs/PLAN.md` and Phase 9 added,** rather than rewriting the Phase 8 row; the two failed cycles stay visible. `features/engine-v4.md` and the restart brief move to `archive/features/` in the activation PR.
(j) **Winter fruits are a content addition, not a check.** The library's Winter-eligible set is exactly banana, papaya, pomegranate; the fruit list is owed by Rajat.
**Flagged to Rajat, not decided:** closing PRs #228 and #229 and deleting the four v4.1 branches and the prototype worktree (destructive; after F1 cherry-picks the `cuisine_neutral` widening); the Winter fruit list; hummus as a main; the per-action prod approvals the runbook names.
**Reversibility:** high; nothing is implemented, and every file written this session is untracked or an uncommitted working-tree edit until the activation PR. **Right-size:** one plan, one spec round, no new tables, one additive field, one new internal mutation and one internal query.

## 2026-09-04  Phase 9 build session 1: wave 0 and wave 1 landed, four scoping calls taken  (features/engine-v6-plan.md §6, §7)
**Stream:** 0, A0, A, B, C, E1, F1, F2, G (EM coordination)
**Context:** first build sitting of Phase 9. The activation PR (#239), F1 (#242), and F2 (#241) merged to `main` on Rajat's instruction; A0 (#240), C (#246), B (#247), E1 (#244), and A (#245, after one send-back) merged into the integration branch `feat/engine-v6`; G's docs rewrite is a draft (#243); D is in flight. Calls the EM took without Rajat, each reversible:
(a) **The integration branch was created off `origin/main` before #239 merged,** so wave 1 could spawn in the same sitting; engineers read the phase docs from the main dir by absolute path until `origin/main` was merged into the branch (merge commit, per hotspot H21). Reversibility: none needed; the branch history is one merge commit richer.
(b) **F1 drops the salvaged patch's `pairsWith: [Kadhi]` hunk on fish tikka and adds `cuisine_neutral` to fish tikka by hand.** `pairsWith` is not a schema field on `main` and spec §13 and §14 item 4 exclude it; the widening the plan names is five dishes, and the household's Saturday pairing of Thai pineapple fried rice with fish tikka (spec §5.4) needs fish tikka in the dry-protein partner pool, which stream B keys on that tag. Right-size: one tag line.
(c) **F2 authors four dishes, not five.** "Cabbage matar aaloo" is the existing Cabbage peas aloo (id 23; matar is peas); a second file would split one dish's record across two ids. The prod re-point for that slot targets 23. F2 also added one catalog row, `Urad Dal`, distinct from `Black Urad Dal` (different pulses, different macros and buy list).
(d) **Cross-stream shapes fixed in the briefs, not in `types.ts`:** `Plate`, `PoolEntry`, the `PoolProvider` callback, and `Repair`, so A, B, and C could build in parallel with no shared file beyond A0's contract. A's seed takes the structural id set as a parameter and B owns the single `isStructuralPoolDish`; C returns repairs and D applies the ledger refund and charge. D dedupes the two identical `Plate` definitions (B's and C's) and narrows `RecordStats.eatenCount` and `rate` to partial records, the one sanctioned post-A0 `types.ts` edit (hotspot H15).
(e) **Spec clarifications written into `features/engine-v6.md` this sitting, both non-rule:** §2.2's fish tikka illustration now reads under Saturday scoping (6 of 36 weekday-lunch occasions plus 1 of 8 Saturdays), and §3.1 states that each replayed week is replayed in its own season. The second came from the A review: the first implementation used the generating week's season for every replayed week, which would have banked dozens of servings for a Summer fruit across Monsoon and Winter; the fixture measures 36 banked servings before the fix and a frozen deficit after.
(f) **Protein salads stay out of the dry-protein partner pool** (spec §5.1 names Category Keto or Dry dish); the `cuisine_neutral` tag on the three salads matters only for the standard international plate's companion register filter. No cuisine-register filter on the partner itself (§5.4's fried rice with fish tikka).
(g) **Saturday dessert is cap-protected** in the rewritten `docs/engine.md` (G's call, accepted): spec §5.4 and gate threshold 5 require dessert on every Saturday, so a droppable dessert contradicted the gate.
**Findings recorded, not decided:** CI has no spec-code parity step and no Convex codegen step although `docs/engineering.md` §15 gates 2 and 6 and `CLAUDE.md` say it does (G restates the pairing as review-held; the §15 wording is corrected at G's final pass, lane widened to that sentence); two Active dishes share the name "Paneer bhurji" (ids 13 and 106); the dev deployment still holds the retired `nextWeekQueue` and `comments` tables and had `save_next_week` rows (dev only, cleared by E1); `engine/test/data/bake.test.ts` deletes the baked library in `afterAll`, so local gates must run in CI order; the shared session scratchpad silently crossed one PR body between two streams before filenames were prefixed by stream.
**Reversibility:** high; every call is a data line, a brief, or a doc sentence. **Right-size:** no new abstraction; one merge commit; two spec sentences.

## 2026-09-07  Phase 9 gate fix cycles 1 to 3: six thresholds settled by measurement, two deferred to the real record  (features/engine-v6.md §11, PRs #251, #253, #254)
**Stream:** D2, D3, D4 (EM triage after each `npm run gate`)
**Context:** the first gate run on the built engine (8-week fixture, 60 weeks self-feeding, weeks 20 to 60) passed 6 of 12 thresholds self-feeding and 5 of 12 frozen. Three fix cycles later it passes 9 of 12 self-feeding and 10 of 12 frozen. Every change followed §11's order of work: amend the spec with the measured reason, then fix, then re-run.
**Decisions (EM, each an arithmetic or definitional clarification, not a taste rule):**
(a) **Families under four record rows are reported, not gated, on thresholds 1 and 12** (mutton, two rows, read +86 percent while eleven families passed).
(b) **Threshold 4 counts a chutney lock per dish** (the category sat at 22 of 41 Mondays by construction) and **exempts a dish at or above 0.4 of its role's weekly slots** (plain roti at 0.473: a preference-free spread puts one weekday over half the horizon 89.7 percent of the time; the engine's worst weekday sat at the random mean).
(c) **The §3.2 presence-ledger trigger fired** for the weekday companion (+34 percent self-fed, +38 frozen) and the Saturday third item (100 against 75); the frozen failure showed the wide-pool maximum effect, not drift. Presence is defined by role on both sides: a §5.1 floor append is not the slot (metering by plate size let 13 of 108 charged occasions be floor appends and starved raita, threshold 1 at -25.5 percent); the record side reads each plate structurally.
(d) **The exploration slot's reserved weekday memory became the rule** (plain roti held Friday 21 of 41 weeks with the pick taking the leftover day) and the weekday is reserved first, not assigned last (five plates fill five weekdays, so assigned last it would have one day and the memory would be inert).
(e) **Threshold 12 gates on a shift larger than the family's Poisson counting noise**; the 10 percent figure is reported. At 41 weeks the quietest family's noise is 18.9 percent, so the fixed bound measured noise.
(f) **Two implementation gaps fixed, no spec change:** §9's within-week fruit exclusion (mango three times a week, five consecutive-day repeats) and the frozen run's exploration candidacy (frozen eaten counts re-offered served explorations weekly; frozen threshold 2 rose from 52.5 to 70.0 percent).
(g) **`starRoleShare` rejected on measurement:** admitting a dish to the star pool only when half its record rows are stars drove threshold 2 from 62.5 to 57.5 percent, was absorbing (fish tikka out from week 1 to 60), and was decided by single record rows (22 of 48 dishes). Removed.
**Decisions (Rajat, 2026-09-06):** threshold 5 (Saturday treat repeats under self-feeding; frozen passes; three leading treats above 1 of 8 after the ratchet on a one-occasion scope) stays failing until the gate runs on the real record; no prod read before the integration merge; on 2026-09-07, merge the integration branch with thresholds 2 (62.5 against 65, baseline 77) and 5 open, the first real-record gate run being the cutover condition.
**Open, carried to the real-record run:** threshold 2 (fish tikka takes 27 stars self-fed against 14 frozen because one scope ledger spends its deficit in whichever role opens first); threshold 5; persisting the role beside each `generatedPlan` pick, which would make the two presence readings exact (5.6 percent apart) at the cost of a §12 and schema change.
**Reversibility:** high; every amendment is a threshold definition or a pre-authorised trigger, and the engine changes are fixtured. **Right-size:** three short cycles, one variant tried and deleted, no new table.

## 2026-09-07  Engine v6 cutover: integration merged, six custom picks re-pointed, first v6 week generated  (features/engine-v6-plan.md §8, PR #256)
**Stream:** I (EM with Rajat's per-action approvals)
**Context:** Rajat merged the integration branch with thresholds 2 and 5 open ("Merge", "Go ahead"), approved the three prod actions in order, and, after the real-record gate report, the first generation.
**Runbook as executed:** Deploy Convex and CI green on `8546d14`, prod 200; `recordExport:exportRecord` (10 weeks, 2026-06-08 to 2026-08-24, no generated plans); six `promoteCustomPick` runs (2026-07-06 Wed breakfast Red Sauce Pasta to 284; 2026-07-20 Tue lunch Paneer Manchurian to 287; 2026-08-10 Tue lunch Stuffed capsicum to 288, Wed lunch Cabbage matar aaloo to 23, Sat lunch Dosa to 285 and Atta halva to 286), re-export confirming all six; `npm run gate` on that export, committed as `features/engine-v6-gate-report.md`; `generateCurrentWeek` for 2026-09-07 (version 1, zero incidents, 30 picks, plan persisted).
**Decisions (Rajat):** (a) merge before a fully passing gate, the first real-record run being the cutover condition; (b) generate with the real-record run at self-feeding 8 of 12 and frozen 10 of 12, the new failures being noise-level.
**Decisions (EM):** (c) **threshold 8's band is now relative to the record's own international rate (within 40 percent)** rather than the fixed 0.75 to 1.75 set from the fixture: on the prod record the household runs 0.93 a week, the engine 0.88 on average, and one 10-week window at 0.50 is ten placements of counting noise. (d) The frozen run's specialty-roti starvation (5 rows, -41 percent; beetroot roti unserved 20 weeks) is a thin-carb bias under frozen rates that the self-feeding run does not show (+17.6 percent); queued as the next harness cycle, not a cutover blocker. (e) The real-record report replaces the fixture report as the committed gate artifact.
**Observations from the first week, for the slow loop:** rice on Thursday (curd rice) and Friday (steamed rice), the soft rule accepting the pair; Thursday's egg anchor satisfied by boiled eggs riding beside the pinned avocado toast.
**Residuals:** `app/web/e2e/smoke.mjs` still expects a Changes tab (Phase 7 moved it) so its prod pass reports a stale failure; the picker's dev smoke was run by the EM; MAINTENANCE.md, ADDING-DISHES.md, development.md §6, and the slow-loop brief describe `poolCoverageReport` and the seed history (reconcile-ops), and the monthly v6 monitor table is owed there.
**Reversibility:** the merge is one squash commit; the re-points are six slot edits; the week is a draft the household edits. **Right-size:** no new mechanism at cutover; one threshold band re-based on the record it measures.

## 2026-09-07  Phase 9 close-out: winter fruits not needed, hummus a lunch main, weekly generation paused, EM work completed  (archive/features/engine-v6-plan.md)
**Stream:** EM (phase close-out)
**Context:** with engine v6 live, Rajat settled the three items the plan left open and asked for the remaining EM work to be completed, gate thresholds excepted.
**Decisions (Rajat, 2026-09-07):** (a) **No Winter fruit content batch (F3 cancelled).** The fruit slot's §9 exhausted-pool rule admits every Active in-season fruit, candidates included, so the season is covered without new dishes. (b) **Hummus is a lunch main (F4):** Category Accompaniment to Dry dish, one field, nothing else; it enters the weekday star pool through its two record rows. (c) **Weekly generation stays paused;** no run for 2026-09-14 until Rajat says. (d) **Gate thresholds 2 and 5 are left as they stand** on the record; nothing is amended or built for them.
**Decisions (EM, completing the list):** (e) the frozen run freezes the presence rate too (PR #261; it also cured the frozen-run specialty-roti starvation, whose cause was the carb slot partitioned by affinity against one carb ledger; a per-affinity carb presence ledger for the residual 5.3-placement rice-versus-chapati imbalance is recorded here as a candidate, not taken); (f) the UI smoke harness catches up to the Yours tab and the profile sheet (PR #258) and `app/web/e2e/` joins the ops reconcile's scope so it cannot go stale silently again; (g) the operational and canonical reconciles ran (PRs #260, #262) with the monthly engine monitor written into `MAINTENANCE.md` §1.9; (h) the v4 prototype worktree and branch, `proto/engine-v4-simulation`, and seven squash-merged remote branches were deleted, and the dev deployment's retired `nextWeekQueue` and `comments` rows cleared; (i) the two Active dishes named "Paneer bhurji" (ids 13 and 106) are a deliberate Lunch and Breakfast pair and both stay, with id 106's empty ingredients table and the question of an HP tag on hummus (22.4 g protein a person against the 20 g bar) recorded as content items for Rajat; (j) the phase closes with the spec and plan under `archive/features/` and the evidence trail under `archive/features/engine-v6/`, the gate report staying at `features/engine-v6-gate-report.md` as the harness's living output.
**Retro intake (MAINTENANCE.md §6, entries of 2026-09-07):** stale smoke harness, fixed (#258 plus the ops-scope line); shared scratchpad, fixed by a brief line, the per-stream scratchpad itself being a Claude Code matter (wont-fix here); subagent losses, wont-fix (platform), mitigations already in the brief; docs claiming absent CI checks, fixed (#256, #262) with a real parity step surfaced to Rajat as a CI change; the contradicted ownership point, triaged to the next phase plan's hotspot table; the bake test's afterAll, fixed by a brief line (gates in CI order), the test itself left for a chore.
**Reversibility:** high; the tag and the archive moves are the only irreversible-in-spirit acts and both are conventional. **Right-size:** one docs PR closes the phase.

## 2026-09-07  The engine-evolution process becomes a resumable skill  (EVOLVING-THE-ENGINE.md, .claude/commands/evolve-engine.md)
**Stream:** EM (chore/evolve-engine-skill, PR #264)
**Context:** Rajat asked for the process that produced engine v5 and v6 to be captured so he can trigger it whenever he judges the time right, with the rounds running without him and nothing lost if the account's usage limit cuts a session.
**Decisions (Rajat):** the mechanism is a slash-command skill with a full operational document of its own, not a section of `MAINTENANCE.md`; the structure is the seven steps with three review rounds (differ, first-principles critic, a simplicity-versus-coverage debate, a decider); he is involved only at the household round (step 6) and at the cutover approvals; "no change this round" is a valid outcome; every agent runs on the highest-intelligence model at highest effort.
**Decisions (EM):** (a) **the decider is the household's proxy**, with Rajat's mandate written into its brief (weigh a change by the recorded hand edits it removes, refuse minor optimisation, decide only from what the household did and said, park taste for step 6 and proceed conservatively); (b) **the decider runs twice per round**, a grounding pass that writes the numbered decision list the debate argues over (the shape of the v6 decision brief) and a decision pass after the debate, so the agenda is a committed artifact rather than EM reasoning; (c) **the resolution audit lives in the differ from round two**, at the cost of a wider reading list (the previous menu and reports), with the spec still forbidden; (d) **the run folder is the only state**: one artifact per step under `features/engine-<version>/`, a `RUN.md` manifest with `resume-at`, a commit and push after every step, the simulator source committed and unlinted, the snapshot zip ignored; (e) **the usage-limit protocol**: on a 429 naming a reset time the EM records `resume-at`, commits, and schedules its own wakeup (`CronCreate` one-shot at reset plus ten minutes; `ScheduleWakeup` chained hourly as the fallback), and a dead session resumes with `/evolve-engine resume`; (f) **step 1 reads production through two read-only commands under one approval** (the engine-shaped export and the snapshot export, the latter carrying every `manualChanges` row and the custom picks' labels), and Rajat's invocation is that approval; (g) grounded in v5 and v6 only; v4 and v4.1 are named nowhere in the process.
**Flagged for `/reconcile-docs`:** `docs/development.md` §2 branch naming (`evolve/engine-<version>`), `docs/engineering.md` §14 layout (the new root doc, `.claude/evolve/`).
**Reversibility:** high; documents and briefs, no runtime code. **Right-size:** one spec, one command, eleven role briefs, two templates, four hygiene lines.

## 2026-09-07  The maintenance machinery becomes one skill, /maintain, with five passes  (archive/features/maintenance-skill-structure.md, MAINTENANCE.md, .claude/skills/maintain/)
**Stream:** EM (docs/maintenance-skill-structure #266; chore/maintain-skill #269, chore/dislike-writeback #268, chore/evolve-reads-requests #267)
**Context:** with `/evolve-engine` shipped as its own skill (#264), Rajat asked for the maintenance workflows (the slow loop, the two reconciles, the retro intake) to be kept separate from engine evolution and designed as one skill. The EM grounded the design in three inputs: the #265 review, an independent clean-room study of every maintenance run since June by an agent forbidden from reading the review, and the current Claude Code documentation on skills, arguments, and subagents. The clean-room study independently arrived at the same five passes; it disagreed with the review on six specifics, and the EM sided with it on each.
**Decisions (Rajat, 2026-09-07, "Go ahead" on the twelve recommendations of the structure document §11):** (D1) one `/maintain` command with five passes, each invocable alone; (D2) the two reconciles merge into one `docs` pass; (D3) retro is its own pass, run inline by the EM; (D4) the boundary is what the engine reads versus how it decides, with no rule-edit escape hatch inside maintenance (the fast loop, a `/new-stream` feature PR with the full gate green, is the path for a rule Rajat wants shipped without an evolution); (D5) a defect fix needs a failing test naming the spec clause, else it is refused; (D6) the handoff ledger is `data/engine-requests.md`, read by the evolve critic and decider; (D7) one manifest-style `.maintenance-state` with a deferred list, `.retro-state` deleted; (D8) the cheap spec-code parity CI check is approved in principle and not yet built; (D9) the dislike write-back mutation is built first; (D10) the first sitting consumes the pre-v6 signal backlog with a stated cutoff; (D11) `/maintain` uses the skill-folder layout now, `/evolve-engine` migrates later as a mechanical move; (D12) the names `MAINTENANCE.md` and `slow-loop/<date>` are kept because code depends on them.
**Decisions (EM):** (a) **a new tag value is evolution**, because tags are a closed enum and a new value is a schema change plus a rule that reads it; applying an existing value to more dishes is data. (b) **The ledger is read by the critic and the decider from round one, never by the rulebook author or the spec author**, whose clean room forbids anything that references engine behaviour; the EM snapshots the open entries into the run folder at step 1 so a run is deterministic against a growing ledger. (c) **The retro pass's window is by status, not date**: every entry not closed regardless of date plus entries appended since the marker, because two 2026-08-18 entries were already invisible to a date window. (d) **Signals, health, and docs are spawned as subagents; retro and hygiene run inline**, since one is the EM's judgment about its own ledger and the other is five minutes of mechanical checks. (e) **The docs pass carries a standing-checks list beside its window**, because four false claims survived two window-scoped passes. (f) **The mark-applied contract stays at `MAINTENANCE.md` §3** rather than moving to the end, so the three live `§3` pointers in the script and the engineering doc stay valid. (g) **A ledger's instructional header is spec and the docs pass may correct it; its entries are history and are never rewritten.** (h) **No usage-limit wakeup is imported** from the evolve skill: a sitting is hours, and a dead one is resumed at the next.
**Not taken:** the review's `features/engine-requests.md` location (the folder is empty between features); the study's rule-edit escape hatch with Rajat's per-PR approval (an unmeasured rule edit is the v4 failure, and the fast loop already exists).
**Reversibility:** high; documents, briefs, one internal mutation, one optional schema field. **Right-size:** one structure document, one skill, three chores, no new table.

## 2026-09-07  First /maintain sitting: interpretation calls on the skill's first run  (archive/maintenance/2026-09-07/, PRs #271 and #272)
**Stream:** EM (the sitting; passes 1 to 3 as spawned agents, 4 and 5 inline)
**Context:** Rajat said "Merge and /maintain". The skill is `disable-model-invocation`, so the EM read `SKILL.md` and ran the sitting from it. Five calls were needed where the spec was silent or two rules met.
**Decisions (EM):** (a) **A Convex CLI table dump is a read of the same class as a query read.** No query lists `dishDislikes`; rather than leave the table unread, the signals pass read it with `npx convex data dishDislikes --prod` (empty), and a listing query is filed as `chore/maintain-queries` so the next sitting reads all three tables the same way. (b) **The monitor's swap measures may read the signals pass's raw export** for counting only, never for clustering, since the health brief both forbids the queued tables and requires the swap rows; a by-week query of any status is in the same chore because the queued export saw 4 of 8 weeks. (c) **Each branch archives its own half of the sitting folder and the docs branch starts from the slow-loop branch's state file**; written into `SKILL.md` step 8 in #272 and logged in `RETRO.md`. (d) **Closed-but-unmerged remote branches are Rajat's, not the hygiene pass's:** the brief's one sanctioned deletion is a branch whose PR merged; `data/photos-4` and `data/photos-7` hold an unmerged commit each and are listed instead. (e) **Two same-day sittings would reuse a branch name** (`docs/maintenance-2026-09-07` was also #262's head today); accepted as a one-off, not logged.
...
## 2026-09-16  Canonical docs rewritten from scratch with section numbers held; code wins over every stale claim  (PR #278)

**Context:** Rajat asked for `docs/product.md`, the design contract, `docs/engineering.md`, and `docs/development.md` to be a perfect reflection of the current product, written from scratch rather than patched, under the `/maintain` docs-pass rules. Reading the code turned up claims the docs had carried for months that were never or no longer true (the swap meal-time guard, a `swiggyCarts` table, an auto-recovery middleware, per-PR Convex previews, a hook that rejects every main-directory commit).

**Options considered:** (a) rewrite each document with its existing section skeleton, so the roughly two hundred `§` pointers in `MAINTENANCE.md`, the briefs, `ADDING-DISHES.md`, and code comments keep resolving; (b) restructure freely and chase every pointer; (c) treat "design" as `docs/engine.md` and rewrite the rules spec too.
