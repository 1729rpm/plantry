...
// ---------- Dish action sheet ----------
function DishActionSheet({ entry, favorited, onFavorite, onReplace, onDetails, onDelete, onClose }) {
  ...
  return (
    <Sheet onClose={onClose}>
      ...
      <div style={{ marginTop: 6 }}>
        ...
        {entry.key && row(favorited ? 'Remove from favorites' : 'Mark as favorite', favorited ? 'In every week\'s menu' : 'Keep it coming every week', onFavorite)}
...
//          'explore' (use this week, wishlist, not for me; also used from Yours),
//          'replacePreview' (picked from the swap picker: use this dish)
function DishDetailSheet({ dishKey, context, includeRecipe, wishlisted, onToggleRecipe, onReplace, onDelete, onUse, onWishlist, onReplaceWith, onDislike, onClose, defaultOpen }) {
    ...
    <Sheet onClose={onClose}>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
        ...
        {context === 'explore' && (
          <React.Fragment>
            ...
            <QuietButton onClick={onWishlist} style={{ flex: 1 }}>{wishlisted ? 'Wishlisted ✓' : 'Add to wishlist'}</QuietButton>
          </React.Fragment>
        )}
      ...
      {context === 'week' && (
        <div style={{ textAlign: 'center', marginTop: 12 }}>
          <button onClick={onWishlist} style={{ fontSize: 13.5, fontWeight: 600, color: wishlisted ? OT.color.green : OT.color.sub, padding: '12px 16px', minHeight: 44, fontFamily: OT.font.sans, borderBottom: '1.5px dashed ' + OT.color.line }}>{wishlisted ? 'On your wishlist ✓' : 'Mark as wishlist'}</button>
        </div>
      )}
      {context === 'explore' && (
        <div style={{ textAlign: 'center', marginTop: 12 }}>
...
// ---------- Profile ----------
// Opened from the avatar on the Menu header. Username choice plus the changes log.
function ProfileSheet({ identity, activity, onSwitch, onChanges, onClose }) {
  const { Sheet, Avatar } = window;
  const n = activity.filter((a) => a.kind && a.kind !== 'comment').length;
  const other = identity === 'Rajat' ? 'Tuhina' : 'Rajat';
  const row = (label, hint, onClick) => (
    <button onClick={onClick} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '15px 4px', borderTop: '1px solid ' + OT.color.line, minHeight: 52 }}>
      <span style={{ fontSize: 16, fontWeight: 600, color: OT.color.ink, fontFamily: OT.font.sans }}>{label}</span>
      <span style={{ fontSize: 12.5, color: OT.color.sub, fontFamily: OT.font.sans }}>{hint}</span>
    </button>
  );
  return (
    <Sheet onClose={onClose}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '6px 0 14px' }}>
        ...
        <div>
          <div style={{ fontFamily: OT.font.serif, fontSize: 20, fontWeight: 700 }}>{identity}</div>
          <div style={{ fontSize: 12.5, color: OT.color.sub, marginTop: 1, fontFamily: OT.font.sans }}>Edits carry your name</div>
        </div>
      </div>
      {row('Switch to ' + other, 'Change who is editing', onSwitch)}
      {row('Changes to this week', n === 0 ? 'None yet' : n + (n === 1 ? ' change' : ' changes'), onChanges)}
    </Sheet>
  );
}

// ---------- Changes log (moved from the old Changes tab into the profile) ----------
function ChangesLogSheet({ activity, onClose }) {
  const { Sheet, Card, Avatar } = window;
  const n = activity.filter((a) => a.kind && a.kind !== 'comment').length;
  return (
    <Sheet onClose={onClose} maxHeight="92%">
      <div style={{ fontFamily: OT.font.serif, fontSize: 20, fontWeight: 700, marginBottom: 2 }}>Changes</div>
      <div style={{ fontSize: 13, color: OT.color.sub, marginBottom: 12, fontFamily: OT.font.sans }}>{n > 0 ? `${n} ${n === 1 ? 'change' : 'changes'} to this week's menu` : "Everything done to this week's menu"}</div>
      {activity.length === 0 && (
        <div style={{ padding: '24px 0', textAlign: 'center', color: OT.color.sub, fontSize: 14, fontFamily: OT.font.sans }}>No changes yet. The week is as the menu was made.</div>
      )}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {activity.map((a, i) => (
          <Card key={i} style={{ padding: '14px 16px', display: 'flex', gap: 12, background: OT.color.bg, border: 'none' }}>
            <Avatar who={a.who} size={28} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 14.5, fontWeight: 600, lineHeight: 1.35, fontFamily: OT.font.sans }}>{a.text}</div>
              <div style={{ fontSize: 12.5, color: OT.color.sub, marginTop: 3, fontFamily: OT.font.sans }}>{a.who} · {a.when}</div>
              {a.reason && <div style={{ fontSize: 13, color: OT.color.ink, marginTop: 7, background: OT.color.surface, borderRadius: 10, padding: '8px 12px', fontFamily: OT.font.sans }}>"{a.reason}"</div>}
            </div>
          </Card>
        ))}
      </div>
...
// ---------- Add a favorite (search the library, or save a custom name) ----------
function AddFavoriteSheet({ favorites, onPick, onPickCustom, onClose }) {
  const { Sheet, SearchField, DishRow } = window;
  const D = window.PlantryData.DISHES;
  const [q, setQ] = useOvState('');
  const taken = new Set(favorites.filter((f) => f.key).map((f) => f.key));
  const results = Object.keys(D).filter((k) => !taken.has(k) && D[k].name.toLowerCase().includes(q.toLowerCase()));
  return (
    <Sheet onClose={onClose} maxHeight="92%">
      <div style={{ flexShrink: 0 }}>
        <div style={{ fontFamily: OT.font.serif, fontSize: 20, fontWeight: 700, marginBottom: 2 }}>Add a favorite</div>
        <div style={{ fontSize: 13, color: OT.color.sub, marginBottom: 12, fontFamily: OT.font.sans }}>A favorite gets a place in every week's menu</div>
        <SearchField value={q} onChange={setQ} placeholder="Search the library, or type a dish" autoFocus />
        {q.trim() && (
          <button onClick={() => onPickCustom(q.trim())} style={{ width: '100%', textAlign: 'center', fontSize: 14, fontWeight: 600, color: OT.color.accent, border: '1.5px dashed ' + OT.color.accent, borderRadius: 14, padding: '12px 0', marginTop: 10, fontFamily: OT.font.sans }}>Add "{q.trim()}" as a favorite</button>
        )}
      </div>
      <div style={{ marginTop: 6 }}>
        {results.map((k) => (
        ...
        {results.length === 0 && !q.trim() && <div style={{ padding: '20px 0', color: OT.color.sub, fontSize: 14, fontFamily: OT.font.sans }}>The whole library is already in your favorites.</div>}
        {results.length === 0 && q.trim() && <div style={{ padding: '20px 0', color: OT.color.sub, fontSize: 14, fontFamily: OT.font.sans }}>Nothing in the library matches. Add it as its own dish above.</div>}
      </div>
    </Sheet>
  );
}

Object.assign(window, { DishActionSheet, DishDetailSheet, GenericPickerSheet, FruitPickerSheet, ReasonDialog, DayPickerSheet, SharePreviewSheet, ProfileSheet, ChangesLogSheet, AddFavoriteSheet, orderLibrary });
