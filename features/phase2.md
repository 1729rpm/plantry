...
| B Engine | in progress | TBD | Slices 1-4 shipped (eligibility, schedule, composition, priority). Priority's §4 step 3 is a no-op placeholder until §6. Next: cap (§5), then consolidation (§6 which retires the §4 step 3 placeholder), then `generateWeek` and `rankCandidatesForSlot`, then simulation harness. |
...
| D Frontend | in progress | TBD | Slice 1 (app shell, passcode gate, identity picker, read-only current-week view with library enrichment, offline cache) shipped 2026-06-09 in PR #13. Next: swap UI with engine-suggested alternatives, then custom one-off entry, then comment affordances, then grocery list. `userProfiles` Convex sync also still queued. |
