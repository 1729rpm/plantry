...
| C Convex backend | in progress | TBD | Slices 1-3 shipped (schema + read-only queries + seed, three browser-callable mutations, `generateCurrentWeek` internalMutation). Next: `swapDish` and `finalizeWeek`. Auto-recovery middleware and `getGroceryList` queued. |
