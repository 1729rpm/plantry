...
| B Engine | in progress | TBD | Slice 1 (eligibility) shipped in PR #5. Slice 2 (schedule per §2) shipped 2026-06-08 in PR #8. Next: composition (§3), then priority (§4), cap (§5), consolidation (§6), `generateWeek`, `rankCandidatesForSlot`, simulation harness. |
...
| D Frontend | ready to spawn | TBD | Convex client codegen exists; §5 item 12 (`weekArchive` shape) resolved. First slice: passcode gate + identity picker + read-only current-week view rendering `getCurrentWeek` output. |
