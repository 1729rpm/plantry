...
| C Convex backend | in progress | `../plantry-stream-C` on `feat/C-generate-current-week` | Slices 1-2 shipped. Slice 3 (`generateCurrentWeek` internalMutation bridging engine `generateWeek` to the `currentWeek` table, with incident logging) spawned 2026-06-09. Once merged, the ship-checklist box "Current week renders end to end" goes green after one EM `npx convex run` smoke. `swapDish` and `finalizeWeek` next. Auto-recovery middleware + `getGroceryList` queued. |
