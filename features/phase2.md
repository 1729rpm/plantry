...
| B Engine | in progress | `../plantry-stream-B` on `feat/B-priority` | Slices 1-3 shipped. Slice 4 (priority §4 steps 1, 2, 4 fully; step 3 placeholder until §6) spawned 2026-06-09. Cap (§5), consolidation (§6), `generateWeek`, `rankCandidatesForSlot`, simulation harness queued. |
| C Convex backend | in progress | `../plantry-stream-C` on `feat/C-comment-profile-oneoff-mutations` | Slice 1 shipped in PR #6. Slice 2 (`addComment`, `setUserProfile`, `addCustomOneOff` mutations, no engine integration) spawned 2026-06-09. `swapDish` and `finalizeWeek` wait for Stream B's `generateWeek`/`rankCandidatesForSlot`. Auto-recovery middleware and `getGroceryList` (depends on B §6) queued. |
