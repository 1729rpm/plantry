...
| B Engine | in progress | TBD | Slices 1-5 shipped (eligibility, schedule, composition, priority, consolidation). Priority's §4 step 3 now reorders by `IngredientLedger` when supplied. Next: cap (§5), then `generateWeek` and `rankCandidatesForSlot` (compose the full pipeline), then the 5-week simulation harness with the four known-bug replays. |
