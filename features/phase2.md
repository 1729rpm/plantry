...
| B Engine | in progress | `../plantry-stream-B` (`feat/B-cap`) + `../plantry-stream-A` (`feat/B-generateweek`) | Slices 1-5 shipped. Slice 6 (cap §5) and slice 7 (`generateWeek` + `rankCandidatesForSlot` composing the pipeline) spawned in parallel 2026-06-09; slice 7 stubs cap as identity until slice 6 lands then rebases. Simulation harness queued after. |
| C Convex backend | in progress | TBD | Slice 1 (schema + read-only queries + seed) shipped in PR #6. Slice 2 (`addComment`, `setUserProfile`, `addCustomOneOff` mutations + shared `assertAuthor`) shipped 2026-06-09 in PR #15. `swapDish` and `finalizeWeek` wait for Stream B's `generateWeek`/`rankCandidatesForSlot`. Auto-recovery middleware and `getGroceryList` (depends on B §6) queued. |
| D Frontend | in progress | TBD | Slices 1 and 2 shipped (read-only week view, then identity Convex sync + inline custom one-off + comments composer + comments list). PR #17 also renamed C's hyphenated mutation file names and unblocked the Deploy Convex action that had failed on PRs #15 and #16. Swap UI waits for B `rankCandidatesForSlot` + C `swapDish`. Grocery list waits for C `getGroceryList`. |
...
| 9 | Cloudflare CNAMEs | Auto-configured via the Cloudflare-Vercel integration (no manual CNAMEs needed). | Done. `plantry.mudgal.xyz` and `plantry-dev.mudgal.xyz` both serve over HTTPS. |
| 10 | Vercel token | Superseded. The GitHub-Vercel integration handles frontend deploys directly on push to main; our CI never needed a `VERCEL_TOKEN` secret. | Done (superseded). |
| 11 | GH Actions deploy step | Superseded. Final shape: frontend deploys via the GitHub-Vercel integration (Vercel's own webhook on the repo); backend deploys via the in-repo `Deploy Convex` workflow using the existing `CONVEX_DEPLOY_KEY` secret. Both run on push to main and are verifiable via `gh run list --workflow="Deploy Convex"` and Vercel's deployments tab. | Done (superseded). |
| 12 | `weekArchive` vs `MenuHistoryRow` shape | Align Convex schema to engine; `currentWeek` stays in its short/lowercase form because it is the live plan, not a snapshot. | Done. `app/convex/schema.ts` `weekArchive.rows[]` now uses full-word days and capitalized meals; `docs/engineering.md` §3 updated to match. Stream D unblocked. |
| 13 | Live `data/menu_history.md` references dish id 7 (Rajma) absent from `data/dishes.md` | Rajat: do not re-add Rajma as its own dish; re-seed the history. | Done. Rajma row removed from `data/menu_history.md` (2026-05-04 Thursday lunch); `KNOWN_MISSING_DISH_IDS` exception removed from `engine/scripts/bake.ts`. |
| 14 | `MAINTENANCE.md` em-dashes (project style was no em-dashes) | Rajat clarified: em-dashes are fine in internal docs, banned only in user-facing content. | Done. Rule rewritten in `CLAUDE.md`. `MAINTENANCE.md` left as-is. |
