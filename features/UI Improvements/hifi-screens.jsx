...
const SUMMARY_ORDER = [
  ...
  { kinds: ['nextweek'], one: 'saved for next week', many: 'saved for next week' },
];
      ...
      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }} data-screen-label="Menu, current week">
        <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 24px) 20px 14px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            ...
            <button onClick={onSwitchIdentity} aria-label="Switch person" style={{ flexShrink: 0, marginTop: 2 }}><Avatar who={identity} size={30} /></button>
          </div>
        </div>
          ...
          <div style={{ padding: '6px 16px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {day.fruit && (
              <Card style={{ padding: '14px 16px' }}>
                <SectionLabel style={{ color: ST.color.sub }}>Fruit of the day</SectionLabel>
...
function GroceryScreen({ onTab }) {
  ...
  const fruit = window.PlantryData.GROCERY_FRUIT;
  const groups = [...G, fruit];
    ...
    <ScreenShell>
      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }} data-screen-label="Changes">
        <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 24px) 20px 12px' }}>
          ...
          <div style={{ fontSize: 13.5, color: ST.color.sub, marginTop: 2 }}>{n > 0 ? `${n} ${n === 1 ? 'change' : 'changes'} to this week's menu` : "Everything done to this week's menu"}</div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }} data-screen-label="Explore">
        ...
        <div style={{ padding: '0 16px 16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, alignContent: 'start' }}>
          {visible.map((k) => {
            const d = D[k];
            return (
              <button key={k} onClick={() => onOpenDish(k)} style={{ background: ST.color.surface, borderRadius: 16, border: '1px solid ' + ST.color.line, overflow: 'hidden', textAlign: 'left', display: 'flex', flexDirection: 'column' }}>
                <div style={{ width: '100%', aspectRatio: '16 / 9', overflow: 'hidden' }}>
                  <img src={d.img} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                </div>
                <div style={{ padding: '10px 12px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ fontFamily: ST.font.serif, fontSize: 16, fontWeight: 600, lineHeight: 1.2 }}>{d.name}</div>
...
Object.assign(window, { ScreenShell, GateScreen, IdentityScreen, ChangeSummary, summarize, MenuScreen, DayScreen, GroceryScreen, ChangesScreen, FilterChip, FilterPanel, ExploreScreen });
