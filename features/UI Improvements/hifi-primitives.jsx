...
// Quiet rounded tile standing in for a fruit photo at full dish-row weight.
...
// Fruit of the day row. Rendered at full dish-row weight (matching the live app);
// fruitKey resolves in PlantryData.FRUITS.
...
function DayCard({ day, showImages, onEdit, onView, collapsed, today }) {
  ...
  if (collapsed) {
    ...
    return (
      <Card style={{ padding: '10px 14px', display: 'flex', gap: 12, alignItems: 'center', background: T.color.bg }}>
        <DateBadge short={day.short} date={day.date} muted compact />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13.5, color: T.color.sub, fontFamily: T.font.sans, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{summary}</div>
        </div>
        {onView && (
          <button onClick={onView} aria-label={'View ' + day.day} style={{ flexShrink: 0, fontSize: 12.5, fontWeight: 600, color: T.color.sub, border: '1px solid ' + T.color.line, background: T.color.surface, borderRadius: 999, padding: '7px 16px', fontFamily: T.font.sans, minHeight: 36 }}>View</button>
        )}
      </Card>
    );
  }
...
function TabIcon({ name, color }) {
  ...
  // Changes: swap arrows
  return (
    <svg {...common}><path d="M6 9h12l-3-3M18 15H6l3 3" /></svg>
  );
}

function TabBar({ active, onTab }) {
  const acts = window.PlantryAppActivity || [];
  const changeCount = acts.filter((a) => a.kind && a.kind !== 'comment').length;
  const badges = { Changes: changeCount };
  return (
