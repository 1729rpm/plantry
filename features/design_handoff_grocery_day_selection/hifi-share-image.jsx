...
// Image 1: the week's menu. One card per day, date badge left, meals and fruit right.
            ...
            <div style={{ flex: 1, minWidth: 0, fontSize: 12.5, lineHeight: 1.5 }}>
              {day.skipped ? (
                ...
                <React.Fragment>
                  ...
                  {day.fruit && F[day.fruit] && (
                    <div><span style={{ color: SH.color.green, fontWeight: 700, fontSize: 10.5, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Fruit&nbsp;&nbsp;</span>{F[day.fruit].name}</div>
                  )}
                </React.Fragment>
              )}
            </div>
...
function GroceryShareImage() {
  const G = [...window.PlantryData.GROCERY, window.PlantryData.GROCERY_FRUIT];
  return (
    <ShareFrame>
        ...
        {G.map((g) => (
          <div key={g.group}>
            ...
            <div style={{ columnCount: 2, columnGap: 18, fontSize: 12.5, lineHeight: 1.7 }}>
              {g.items.map((it) => (
                <div key={it.name} style={{ breakInside: 'avoid', display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                  <span>{it.name}</span><span style={{ color: SH.color.sub }}>{it.qty}</span>
                </div>
              ))}
            </div>
          </div>
