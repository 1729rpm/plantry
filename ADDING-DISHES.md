...
- **New catalog row needs:** `Group` (Proteins and Dairy | Pantry | Vegetables | Aromatics and Herbs | Other), `Unit` (g/ml/pcs), and macros (`Protein/Carbs/Fat/Fiber per 100g`) **for macro-relevant groups only** (Proteins and Dairy, Pantry, Vegetables). Aromatics/Herbs and Other may stay blank. `pcs`-unit rows need `Grams per piece`. Set `Special: Yes` if it needs a supermarket/specialty run (it surfaces in the special-sourcing report). Honour the grouping judgment calls documented at the top of `data/ingredients.md` (Onion/Tomato/Lemon are Aromatics, Capsicum/Cucumber are Vegetables, Coconut Milk/Sprout are Pantry, fruit is Other).
