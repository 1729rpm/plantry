...
5. **Drop in the engineer brief.** Write `../plantry-<branch>/.engineer-brief.md` with:
   ...
   - **Run `npm install && npm run bake` before any typecheck / build / test.** A fresh worktree has no `engine/src/data/library.ts`; the bake generates it from the markdown library (it is gitignored), so a typecheck or test run before the bake fails with a confusing "missing module" error that looks like a broken checkout, not a real defect.
   - **Run the gates in CI order and re-bake after any test run.** `engine/test/data/bake.test.ts` deletes the gitignored baked `library.ts` in its `afterAll`, so a typecheck run after the engine tests fails with a misleading missing-module error; the order is bake, typecheck, lint, format:check, lint:css, build, tests.
   - **Prefix scratchpad filenames with the stream letter.** The session scratchpad directory is shared across every sibling stream, so a generic name like `pr-body.md` is silently overwritten by another stream.
