...
`docs/maintenance-2026-09-07`. The docs pass reconciles six CHANGELOG entries into the four canonical
docs and the operational layer: `docs/product.md` §4 Principle 3 says the spec-code pairing is held by
review and not by a CI check; `docs/development.md` §2 defines every branch prefix the briefs use,
§5's card names the evolution-request resolution, §6 is the maintenance trigger describing all five
passes, and §3 step 8 removes a handoff's working copy at close-out; `docs/engineering.md` §14
carries the root layout with `EVOLVING-THE-ENGINE.md` and `.claude/evolve/`; `MAINTENANCE.md` §3.2
matches the dislike mutation's real writes. The retro pass closes or routes six ledger entries and
appends one of its own (each branch of a sitting archives its own half of the artifact folder). The
hygiene pass finds the tree clean and lists three items for Rajat. Artifacts under
`archive/maintenance/2026-09-07/`. (#272)
Why: the first run of the skill, and the catch-up the five passes were built to close.
Updated: none (the docs pass is the reconciliation; its own deferred items are in `.maintenance-state`).

## 2026-09-07  First /maintain sitting, part one: pre-v6 backlog consumed, first monitor baseline

The signals and health passes of the first maintenance sitting, on `slow-loop/2026-09-07`. Signals
consumes 67 queued manual changes as `reviewed_no_change`: 66 from the four weeks before the v6
cutover under the stated cutoff (generated by engine v5, superseded by v6, #256) and one v6-week add
that is a single instance; 0 incidents; the `dishDislikes` table is empty. Health runs the monthly
engine monitor for the first time over the trailing eight served weeks (baseline only, no previous
table; international is the row to watch) and reports thresholds 2 and 5 unchanged, with three
proactive proposals resolving to no change. One `data/changelog.md` entry; no dish, catalog, engine,
or spec change; no evolution request. (#271)
Why: the backlog predated the engine that now generates, and the monitor had never run.
Updated: none.
...
Second composition-from-feedback round, encoding Rajat's 16 manual edits to the 2026-06-29 week as three structural levers (no new standalone rules). Lever A (#201) relocates cuisine from a per-position §4 nudge to a §3 meal-level input: up to two weekday lunches/week compose as a coherent non-Indian "international" form (one non-Indian anchor + at most one same-cuisine-or-`cuisine_neutral` companion, no Indian carb), the §3.2 substitution path pins which anchor lands on which day, and the Indian thali (Menu 1/2) now composes Indian-cuisine dishes only so a lone non-Indian sabzi/dal can never sneak onto a thali; the old §4 step-5 cuisine nudge (`byCuisineDiversity`, `placedNonIndianCount`, `WEEKLY_NON_INDIAN_TARGET`) is deleted and §4 no longer reads cuisine (the §12 "§3 never reads cuisine" line is reversed). Levers B and C (#200) make the breakfast accompaniment dish-driven (a Chilla/Paratha main carries its chutney in any slot, including the Tue/Thu single-pick), give the Indian weekday lunch a 4-item-thali aspiration (Menu 1 gains a second partner, converging with Menu 2), and make the §9 cap role-aware (it drops a companion side before the carb or protein main), so the day-budget is emergent: a 4-item lunch on light-breakfast days, trimmed to 3 when breakfast is full. Three plain proteins are tagged `cuisine_neutral`. Engine-only (no Convex signature or frontend change); the hand-edited live 2026-06-29 week is left intact, so the new rules apply from the next generated week. (#200, #201)
