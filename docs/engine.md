...
1. **Frozen:** rates fixed at the cutover record for the whole horizon. Measures the engine's own bias; a family that fails here needs an engine fix. The run freezes **rates and nothing else**: every scope rate, the eaten counts and occasion counts those rates are computed from, the per-season fruit counts, and the `lastEatenWeek` the cold start backdates from. Everything the record carries as a memory of where a dish went rather than how often it was eaten reads the live record: the weekday-occupation memory, the exploration slot's own weekday memory, exploration candidacy (§7), and the §4 presence rates. Freezing a memory would stall it, so every week of the horizon would resolve the same least-recently-used day and the run would report a slot lock that is an artifact of the harness rather than a property of the engine. Frozen this way, the run measures rate-following bias against a stalled distribution, not a stalled calendar.
