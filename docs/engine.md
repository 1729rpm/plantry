...
4. **Favorites** over non-favorites. A dish on the household's standing favorites list (the `favorites` table, curated in the app; `features/wishlist.md`) ranks above a non-favorite, as a stable partition so steps 5 and 6 still dominate within the favorite and non-favorite groups. This is the §4 preference signal. Weekly cadence is emergent, not scheduled: step 5 (within-week recency) already sinks an already-placed favorite below fresh alternatives, so a favorite lands about once a week with no due-date arithmetic. A weekly promotion budget bounds it: the generateWeek loop counts favorite picks placed so far this week and, once that count reaches `FAVORITE_WEEKLY_CAP` (6), the favorites step no-ops for the rest of the week, so a long favorites list cannot swamp rotation and discovery. An absent or empty favorites set leaves the step a no-op, so a run with no favorites is identical to one without the step. The `preferred` frontmatter field is no longer a §4 input; it stays parsed (§12) but does not affect selection.
5. **Within-week recency.** A dish already placed in an earlier slot of the week being generated is treated as most-recently-used for every subsequent slot's ranking, so it sinks below any fresh (not-yet-placed-this-week) alternative. This is a dominant ordering: unlike step 1's `menu_history.md` recency, it is applied near-last, so neither consolidation (step 3) nor favorites (step 4) can re-promote an already-placed dish above an equally eligible fresh one. It exists because the cross-week history (step 1) is silent on the in-progress week, so without it a single broad pool's top-ranked dish (e.g. the longest-unused HP gravy) wins every Menu 1 slot Mon/Wed/Fri identically. When every candidate has already been placed this week, demoting them all is the same as demoting none, so the pool is returned unchanged and the repeat is allowed (the fresh-alternative fallback, mirroring step 2).
...
- `preferred`: Yes/No. Parsed and retained, but not a selection input: §4 step 4 reads the household's live favorites list (the `favorites` table, `features/wishlist.md`), not this field. It stays on the dish files for reference and possible future use.
