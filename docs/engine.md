...
After §3 composition has produced the candidate set for a slot, rank candidates in this order. Each ranking step breaks ties from the previous. Steps 1 to 3, 5, and 6 rank candidates within a pool; step 4 is different in kind, a placement guarantee applied before ranking rather than a per-pool tiebreak, so the tie-break chain flows from step 3 straight into step 5.
