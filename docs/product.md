...
5. **Changes feed (Changes tab).** A newest-first record of the week: every menu edit (swap, add, custom dish, delete, skip, restore, save-for-next-week), each with its author, timestamp, a plain-language headline, and the quoted reason when one was given. The tab subtitle and a badge on the Changes nav icon show the count of this week's edits. Dish ids resolve to names; no internal label or enum value reaches the screen.
