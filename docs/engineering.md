...
The scriptable floor of this crawl is `app/web/e2e/smoke.mjs` (`npm run test:smoke`): it loads each tab and asserts no horizontal overflow and correctly sized cards. By default it builds and serves `dist/` locally; setting `CRAWL_URL` (plus `VERCEL_AUTOMATION_BYPASS_SECRET` for a protected preview) points it at a deployed URL instead, using the bypass mechanism above. The EM runs the fuller in-depth crawl-and-compare per slice.
