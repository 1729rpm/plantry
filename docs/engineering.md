...
Dish photos are produced by an offline build-time tool, `scripts/generate-dish-photos.mjs`, run by hand outside the app and the CI pipeline. It generates each photo from the cuisine-aware prompt in `data/dish-photos/STYLE.md`. The prompt aims for candid realism: a photo that looks like a real person photographed their own meal, shot at a natural angle (low or three-quarter, not flat overhead) with a shallow depth of field, served in a real everyday vessel that suits the dish (a steel katori or thali, a karahi or kadai, a cast-iron pan, or a plain home plate) against a softly out-of-focus home or restaurant background, in ordinary warm light with honest true-to-life colour. The decisive cue is that the food looks genuinely cooked and a little imperfect, with the correct real texture for its type: a dry dish stays dry, a gravy is thick, opaque, and oil-flecked with pieces half-submerged, rice is loose separate grains, a pudding is loose and soft; shapes are irregular and hand-made, edges uneven, garnish scattered rather than a single centred sprig. This direction was derived from a study of 60 real reference photos against the prior AI set, which read as too clean and styled; it replaces an earlier overhead, plain-plate, centred-mound framing. The prompt is positive-constraint (it names only what is in frame, the dish in its vessel and the blurred context, rather than listing forbidden cutlery, because the FLUX model family barely honours negative instructions); the cuisine is derived per dish from its tag as a bare adjective ("a home-style {cuisine} dish"), Indian by default. Generation parameters are tuned for realism: `cfg_scale` 3.5 (lower guidance renders looser, more natural food), `steps` 40 (more refinement for real texture), 1024² square, and a per-dish base seed derived from each dish's slug so vessels, backgrounds, and angles vary across the library while a re-run reproduces the same set (variety is desired, not uniformity; `IMAGE_SEED` pins every dish to one seed for A/B testing a prompt change). The prompt assembler also substitutes a visually-equivalent synonym for the filter-tripping tokens "fried" and "sweet-salty" before sending (for example "fried rice" becomes "stir-fry rice", "fried onions" becomes "golden browned onions", a bare "fried" becomes "pan-cooked", "sweet-salty" becomes "sweet and savoury"), because the provider's content filter false-positives on those exact tokens and would otherwise reject the render; the substitution touches only the assembled prompt string, never the canonical dish description on disk.
...
- **Grocery image:** the week's buy list, grouped by category in the fixed order Proteins and Dairy, Pantry, Vegetables, Aromatics and Herbs, Other. It renders the same skip-aware `getGroceryList(weekStart)` result the Grocery tab shows, so the two cannot disagree.
...
**Rendering.** The images are generated client-side in the browser, not by a server. The preview slides and the exported PNGs render from the same React components (`app/web/src/components/ShareImages.tsx`), so the preview and the shared image cannot drift. `html-to-image` walks the live DOM of a hidden 360px-wide render of each slide and paints it to a PNG at 3x pixel ratio. There is no Convex action and no headless render in the share path.
