...
Dish photos are produced by an offline build-time tool, `scripts/generate-dish-photos.mjs`, run by hand outside the app and the CI pipeline. It generates each photo from the cuisine-aware prompt in `data/dish-photos/STYLE.md`. The prompt aims for a candid home phone photo: a single helping on a plain everyday ceramic plate or bowl on an ordinary kitchen surface, shot straight down from overhead in soft window light, with true-to-life colours and real, slightly uneven textures. It is deliberately unstyled. This candid framing replaces an earlier styled-stoneware, flat-lay-with-a-prop direction that read as glossy CGI rather than real food. The prompt is positive-constraint (it names only what is in frame, the dish on its plate, rather than listing forbidden cutlery, because the FLUX model family barely honours negative instructions); the cuisine is derived per dish from its tag as a bare adjective ("a dish of {cuisine} home cooking"), Indian by default. Generation parameters are tuned for realism: `cfg_scale` 3.5 (lower guidance renders looser, more natural food), `steps` 40 (more refinement for real texture), 1024² square. The prompt assembler also substitutes a visually-equivalent synonym for the token "fried" before sending (for example "fried rice" becomes "stir-fry rice", "fried onions" becomes "golden browned onions", a bare "fried" becomes "pan-cooked"), because the provider's content filter false-positives on that exact token and would otherwise reject the render; the substitution touches only the assembled prompt string, never the canonical dish description on disk.
