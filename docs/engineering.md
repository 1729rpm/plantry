...
The shareable output is a family of PNGs sent together, not a single image: a menu image and one recipe sheet per dish the user marked "include recipe when sharing" that week. The user shares the week from the Menu's "Share this week" button, which opens a swipe-rail preview (menu, then the recipe sheets) and a Send action.
...
- **Menu image:** a compact single-column ledger, one row per day, with the day name and date badge alongside that day's meals. A skipped day renders "Skipped" in place of its meals. The near-square aspect keeps the export's longest edge under WhatsApp's downscale cap so it ships un-downscaled.
